...
import { DoublyNode, Step } from "../types/LinkedListTypes";
import * as operations from "./operations/doublyLinkedListOperations";
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from "@/components/ui/card";
import { toast } from "sonner";

const DoublyLinkedList = () => {
  const [head, setHead] = useState<DoublyNode<number> | null>(null);
  ...
  const addHead = (value: number) => {
    ...
    const newSteps = [
      ...
      { description: "Set the current head's prev pointer to the new node (if exists)", nodeIndices: [0, 1] },
  ...
  const addTail = (value: number) => {
    ...
    const newSteps = [
      ...
      { description: "Set the new node's prev pointer to the last node", nodeIndices: [newNodeIndex - 1, newNodeIndex] },
    ...
    try {
      ...
      if (position === 0) {
        ...
        newSteps = [
          ...
          { description: "Set the new node's prev pointer to the current node", nodeIndices: [position - 1, position] },
          { description: "Set the next node's prev pointer to the new node", nodeIndices: [position, position + 1] },
  ...
  const removeHead = () => {
    ...
    const newSteps = [
      ...
      { description: "Set the new head's prev pointer to null (if exists)", nodeIndices: nextNode ? [1] : [] },
  ...
  const removeAt = (position: number) => {
    ...
    if (position === 0) {
      newSteps = [
        ...
        { description: "Set the new head's prev pointer to null (if exists)", nodeIndices: head.next ? [1] : [] },
        { description: "The original head node is removed", nodeIndices: [] }
      ];
    } else if (position === nodeArray.length - 1) {
      ...
      newSteps = [
        ...
        { description: "Get the previous and next nodes", nodeIndices: [position - 1, position + 1] },
        { description: "Set previous node's next pointer to the next node", nodeIndices: [position - 1, position + 1] },
        { description: "Set next node's prev pointer to the previous node", nodeIndices: [position - 1, position + 1] },
  ...
  return (
    <div className="container mx-auto py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold mb-2">Doubly Linked List</h1>
        <p className="text-muted-foreground">
          A linked list where each node contains a data value and two references: one to the next node and another to the previous node.
        </p>
      </div>
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>A linked list where each node has references to both next and previous nodes</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div>
                <h3 className="text-lg font-semibold mb-2">Properties:</h3>
                <ul className="list-disc list-inside space-y-1 text-muted-foreground">
                  <li>Each node contains a value, a reference to the next node, and a reference to the previous node</li>
                  <li>The first node's previous reference is null</li>
                  <li>The last node's next reference is null</li>
                  <li>Traversal is bi-directional (forward and backward)</li>
                  <li>Requires more memory than a singly linked list</li>
                </ul>
              </div>
              
              <div>
                <h3 className="text-lg font-semibold mb-2">Advantages over Singly Linked List:</h3>
                <ul className="list-disc list-inside space-y-1 text-muted-foreground">
                  <li>Can be traversed in both forward and backward directions</li>
                  <li>Deletion operation is more efficient</li>
                  <li>Can quickly access the previous element of the current element</li>
                  <li>Reverse traversal is possible without storing previous elements</li>
                </ul>
              </div>
              ...
              <div>
                ...
                <ul className="list-disc list-inside space-y-1 text-muted-foreground">
                  <li><strong>Insert/Delete at Head:</strong> O(1) complexity</li>
                  <li><strong>Insert/Delete at Tail:</strong> O(1) complexity with tail pointer</li>
                  <li><strong>Insert/Delete at Position:</strong> O(n) complexity</li>
                  <li><strong>Search Forward/Backward:</strong> O(n) complexity</li>
                </ul>
              </div>
            </CardContent>
          </Card>
